...
public class ZxhtChargeMonitorJob  implements Job{
	...
	public void execute(JobExecutionContext context)
		...
		List<ZxhtZzCampaignInfo> userIdList = zxhtZzCampaignInfoDao.getChargeMonitorUserId();
		if(userIdList.size()>0) {
			
			for(ZxhtZzCampaignInfo zxhtZzCampaignInfo:userIdList){
				String message = zxhtZzCampaignInfo.getTaobao_user_id()+":"+"定时任务";
				amqpTemplate.convertAndSend(Constants.CHARGEMONITOR_QUEUENAME, message);
			}
